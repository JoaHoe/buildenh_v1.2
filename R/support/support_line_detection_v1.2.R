...
for (i in vec) {
  ...
  #if (B2[i,1] == theta_ind && B2[i,3]/k >= thr_line_seg) { 
  if (B0[i,2] == theta_ind && B0[i,3] == 1003) { 
    cat("i= ", i,"\n")
    print(B0[i, ])
    ct <- ct + 1
  }
...
#determination of transformation parameter by means of orthoimage (large scale)
#measure two control 2 points (left lower, right upper) and one checkpoint (middle)
L1 <- trans_ortho() #three points are plotted and must be measured by klick on courser
...
x
y
theta_img_appr = (theta_ind - 1)*5 #check table 
...
x=785 #point (mean, img_system)
y=2203 #point (mean, img_system) check!
...
i=102 #index in B (value for i must be adapted)
i=as.integer(i)
B[i,]
cat("PC_nr=", i, "\n")
